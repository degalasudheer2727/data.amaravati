 ...
 * Wired to the app's ThemeProvider (data-theme attribute + persistence) rather
 * than the component's original self-managed `.dark` class, so it controls the
 * real light/dark token system used across the site.
 */
  ...
  return (
    <button
      ...
      className={cn(
        ...
        config.track,
        dark ? "!border-[#2d2a4e] !bg-[#1a1838]" : "!border-[#e8d5b7] !bg-[#fef3c7]",
        className,
      )}
      onClick={toggleDark}
      type="button"
    >
      <div
        className={cn(
          ...
          config.knob,
          dark ? config.knobOffset : config.knobStart,
          dark ? "bg-[#e8e6f0] text-[#1a1838]" : "bg-[#ff9500] text-white",
        )}
      >
        <ThemeSunIcon
          className={cn(
